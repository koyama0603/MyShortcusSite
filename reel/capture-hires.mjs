/**
 * Re-captures the reel screenshots at 2x (deviceScaleFactor: 2 → 2560x1600)
 * into reel/shots/. Mirrors the data + steps of ../capture.mjs.
 *   node capture-hires.mjs
 */
...
async function inject(page, data) {
  await page.evaluate(async (data) => new Promise((resolve, reject) => {
    ...
    req.onsuccess = (e) => { const tx = e.target.result.transaction('workspace', 'readwrite'); const put = tx.objectStore('workspace').put(data, 'main'); put.onsuccess = () => resolve(true); put.onerror = reject; };
    req.onerror = reject;
  }), data);
}
...
(async () => {
  const userDataDir = path.join(__dirname, '.pw-hires');
  fs.rmSync(userDataDir, { recursive: true, force: true });
  const context = await chromium.launchPersistentContext(userDataDir, {
    headless: false,
    args: [`--load-extension=${DIST}`, `--disable-extensions-except=${DIST}`, '--no-sandbox'],
    viewport: VIEWPORT, deviceScaleFactor: 2, locale: 'ja-JP', timezoneId: 'Asia/Tokyo', ignoreHTTPSErrors: true,
  });
  try {
    const extId = await getExtensionId(context);
    const extUrl = `chrome-extension://${extId}/my.html`;
    console.log('Extension:', extId, '(DSF2 → 2560x1600)');

    await step('01-overview', async () => { const p = await fresh(context, extUrl); await shot(p, '01-overview.png'); await p.close(); });
    await step('02-create-menu', async () => { const p = await fresh(context, extUrl); await p.click('.toolbar-create-menu button'); await p.waitForTimeout(450); await shot(p, '02-create-menu.png'); await p.close(); });
    await step('03-folder', async () => { const p = await fresh(context, extUrl); await p.locator('.desktop-item').filter({ hasText: '仕事' }).first().dblclick(); await p.waitForTimeout(950); await shot(p, '03-folder.png'); await p.close(); });
    await step('09-desktop2', async () => { const p = await fresh(context, extUrl); await p.click('.desktop-switch-button >> text=2'); await p.waitForTimeout(950); await shot(p, '09-desktop2.png'); await p.close(); });
    await step('10-htmlapp', async () => { const p = await fresh(context, extUrl); await p.locator('.desktop-item').filter({ hasText: 'マイ時計' }).first().dblclick(); await p.waitForTimeout(1700); await shot(p, '10-htmlapp.png'); await p.close(); });
    await step('11-youtube', async () => { const p = await fresh(context, extUrl); await p.locator('.desktop-item').filter({ hasText: 'お気に入りの動画' }).first().dblclick(); await p.waitForTimeout(11000); await shot(p, '11-youtube.png'); await p.close(); });
